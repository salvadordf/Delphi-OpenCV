...
begin
  ...
  ptr := @Reader.ptr;
  ptr^ := ptr^ + elem_size;
  if IntPtr(Reader.ptr) >= IntPtr(Reader.block_max) then
    cvChangeSeqBlock(@Reader, 1);
end;
...
begin
  ...
  if Assigned(Seq^.first) and (Cardinal(index) < Cardinal(Seq^.first^.count)) then
    // (seq)->first->data + (index) * sizeof(elem_type) :
    Result := Pointer(IntPtr(Seq^.first^.data) + index * size_of_elem)
  else
    // cvGetSeqElem( (CvSeq*)(seq), (index) )))
...
begin
  // (((elemtype*)((image)->imageData + (image)->widthStep*(row)))[(col)])
  Result :=
  Pointer(IntPtr(image^.imageData) + image^.widthStep * row + col * size_elemtype);
//  {$IFDEF DELPHI7}
//  Pointer(
//  ({$ENDIF DELPHI7} //
//  {$IFDEF DELPHI2005_UP} //
